...
class SafetyLayer:
    ...
    def __init__(self):
        ...
        self.urgent_symptoms = {
            ...
            "температура 40",
            "температура повече от 3 дни",
            "треска 3 дни",
            "силно главоболие",
            "най-силното главоболие",
            "схванат врат",
            "скован врат",
            "не мога да наведа глава",
            "обрив с температура",
            "петна по кожата с треска",
            "подуване на лицето",
            "подут език",
            "подути устни",
            "жълти очи",
    ...
    def check_safety(self, text: str, include_medical_reasoning: bool = True) -> SafetyCheckResult:
        ...
        for severity, pattern, is_red_flag in [
            ...
            if matches:
                ...
                if is_red_flag:
                    logger.warning(
                        f"{severity.upper()} symptoms detected", extra={"severity": severity, "matched": unique_matches}
                    )
    ...
    def check_safety_with_llm_result(
        ...
        if keyword_result.severity == "emergency":
            logger.info("Hard-coded safety detected EMERGENCY", extra={"matched": keyword_result.matched_symptoms})
            return keyword_result

        # If hard-coded found urgent, return immediately
        if keyword_result.severity == "urgent":
            logger.info("Hard-coded safety detected URGENT", extra={"matched": keyword_result.matched_symptoms})
            return keyword_result
